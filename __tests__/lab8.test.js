describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let shadowRootOfProdItem = await prodItems.getProperty("shadowRoot");
    let shadowRootBut = await shadowRootOfProdItem.$('button');
    // Once you have the button, you can click it and check the innerText property of the button.
    await shadowRootBut.click();
    let innerText = await shadowRootBut.getProperty('innerText');
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for(let i = 1; i < prodItems.length;i++){
      let shadowRootOfProdItem = await prodItems[i].getProperty('shadowRoot');
      let shadowRootBut = await shadowRootOfProdItem.$('button');
      await shadowRootBut.click();
      //getCartCount = await prodItems[i].$('#cart-count');
    }
    let cartCount = await page.$('#cart-count');
    let innerText = await cartCount.getProperty('innerText');
    let textVal = await innerText.jsonValue();
    expect(parseInt(textVal)).toBe(20);
  }, 10000);
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    let innerText = await cartCount.getProperty('innerText');
    let textVal = await innerText.jsonValue();
    expect(parseInt(textVal)).toBe(0);
  }, 10000);
